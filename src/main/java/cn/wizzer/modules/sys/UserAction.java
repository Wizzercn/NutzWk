...
import cn.wizzer.modules.sys.bean.Sys_user_role;
...
public class UserAction extends BaseAction {
    ...
    public String search(@Param("username") String username, @Param("userid") int userid) {
        if (!Strings.isBlank(username)) {
            ...
            return Json.toJson(daoCtl.list(dao, Sys_user.class, Cnd.where("id", "=", userid)));
        }
        return "";
    }
...
//        }
        ...
        if (user.getSysrole()) // 判断是否为系统管理员角色
        {
            ...
            rolec = Cnd
                    .wrap("(unitid is null or unitid='') and id in (select roleid from sys_user_role where userid='"
                            + user.getId() + "') order by location,id asc");
        }
    ...
    public Sys_user info(HttpSession session) {
        Sys_user user = (Sys_user) session.getAttribute("userSession");
        return daoCtl.detailById(dao, Sys_user.class, user.getId());
    }
    ...
    public String updateInfo(@Param("..") Sys_user user,
        ...
        Sys_user olduser = daoCtl.detailByName(dao, Sys_user.class, "id",
                userid);
        ...
        if (hashedPasswordBase64.equals(olduser.getPassword())) {
            if (!"".equals(pass)) {
                ...
                olduser.setPassword(hashedPasswordBase);//演示用时可注释此行
