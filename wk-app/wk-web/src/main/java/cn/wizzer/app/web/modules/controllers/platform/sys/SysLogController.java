...
import cn.wizzer.app.sys.modules.services.impl.SysLogServiceImpl;
...
public class SysLogController {
    ...
    public Object data(@Param("beginDate") String beginDate, @Param("endDate") String endDate, @Param("type") String type, @Param("length") int length, @Param("start") int start, @Param("draw") int draw, @Param("::order") List<DataTableOrder> order, @Param("::columns") List<DataTableColumn> columns) {
        ...
        SysLogService sysLogService2 = new SysLogServiceImpl(Daos.ext(sysLogService.dao(), tableName));
        if (sysLogService2.dao().exists(Sys_log.class)) {
            return sysLogService2.data(length, start, draw, order, columns, cnd, null);
        } else
